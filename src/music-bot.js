...
export function createMusicBot({ token, label }) {
...
function createSession(guild,channel){
  ...
  const connection=joinVoiceChannel({
    channelId:channel.id,guildId:guild.id,adapterCreator:guild.voiceAdapterCreator,selfDeaf:true,group:`${client.user.id}:${guild.id}`
  });
...
async function autoLeaveGuild(guildId){
  ...
  if(!channel || humanCount(channel)===0){
    console.log(`[${label}] 👋 自動退出: ${guild?.name??guildId} / ${channel?.name??s.channelId}`);
